...
class Monkey:
    ...
    def play(self,item): 
        ...
        worry_level = (eval(self.operation))%shared_divisible
...
# A 
file = open(f"{os.getcwd()}/day11/puzzle.txt", "r") 
file = file.read()
monkeys = file.split("\n\n")
monkeys = [read_monkey(m) for m in monkeys]
shared_divisible = math.prod([m.divisible for m in monkeys])

print(shared_divisible)
